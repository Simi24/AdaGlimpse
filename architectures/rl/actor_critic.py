...
class AttentionPooling(nn.Module):
    def __init__(self, input_dim, hidden_dim, norm_layer, num_heads=8):
        ...
        self.attention = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.GELU(), nn.Linear(hidden_dim, num_heads))

        self.output_layer = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), norm_layer(hidden_dim), nn.GELU())
    ...
    def forward(self, latent: torch.Tensor, mask: torch.Tensor):
        ...
        latent = latent.reshape(B, P, self.num_heads, self.hidden_dim // self.num_heads).permute(
            0, 2, 1, 3
        )  # B x H x P x (D / H)
...
class BaseAgentNet(nn.Module):
    def __init__(
        self, input_dims: List[int], hidden_dim: int, norm_layer: nn.Module, patch_num: int, exclude_inputs=None
    ):
        super().__init__()
        ...
        self.input_layers = nn.ModuleList(
            [nn.Sequential(nn.Linear(dim, common_dim), norm_layer(common_dim), nn.GELU()) for dim in input_dims]
        )

        self.pooling = AttentionPooling(
            input_dim=common_dim * len(input_dims), hidden_dim=hidden_dim, norm_layer=norm_layer
        )
...
class ActorNet(BaseAgentNet):
    ...
    def forward(self, mask, patches, attention, coords, observation):
        patches = self.patch_net_conv(patches.reshape(-1, 3, 16, 16)).reshape(
...
class ActorCritic(nn.Module):
    def __init__(
        ...
        if exclude_inputs is not None:
            exclude_inputs = [{"patches": 0, "attention": 1, "coords": 2, "observation": 3}[e] for e in exclude_inputs]
        ...
        self.policy_module = ProbabilisticActor(
            module=TensorDictModule(
                ...
                out_keys=["action", "log_prob", "loc", "scale"],
            ),
            spec=BoundedTensorSpec(minimum=0, maximum=1, shape=action_dim),
            in_keys=["loc", "scale"],
    ...
    def forward(self, tensordict):
        ...
        coords = output.get("loc")
        self.log("coords", coords, on_step=True)
        return output
